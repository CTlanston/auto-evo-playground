...
# ---------------------------------------------------------------------------
# Boundary: negative tokens must be rejected
# ---------------------------------------------------------------------------
...
def test_negative_tokens_raise_value_error(in_tok, out_tok):
    """Negative token counts must raise ValueError — same guard as phantom zero runs."""
    store = {}
    with pytest.raises(ValueError):
...
def test_negative_tokens_do_not_write_to_store(in_tok, out_tok):
    """Store must be untouched when tokens are negative."""
    store = {}
    try:
        record_run("run-neg2", in_tok, out_tok, store)
    except ValueError:
        pass
    assert len(store) == 0
...
# ---------------------------------------------------------------------------
# Boundary: non-int token types (bool, None, str) must be rejected
# ---------------------------------------------------------------------------
...
def test_non_int_tokens_raise_type_error(in_tok, out_tok):
    """Non-int token types (bool, None, str) must raise TypeError."""
    store = {}
    with pytest.raises(TypeError):
        record_run("run-type", in_tok, out_tok, store)
...
def test_non_int_tokens_do_not_write_to_store(in_tok, out_tok):
    """Store must be untouched when token types are invalid."""
    store = {}
    try:
        record_run("run-type2", in_tok, out_tok, store)
    except TypeError:
        pass
    assert len(store) == 0
